...
> **중간 점검** </br>
     ...
     >    > ＊ __계시판 수정사항
          └> - [ ] 계시판 detail에서의 별점주기 부분 수정
          └> - [ ] 계시판에서의 선택박스(소형견에게 or 중형견에게 추천넣기) 
          └> - [ ] 계시판의 정보를 저장할때 DB에 이미지정보를 우리가 직접 포함시켜줘서 파일업로드마냥 DB에 저장을 시켜준다 그러면 편하게 값을 가져올수있다.
          └> - [ ] 카테고리 버튼의 수정__
          --------
     >    > ＊ __HOME 수정사항
          └> - [ ] 유투브 - 강사님(유투브) , 트위터가 (404) , 
          └> 2. 메뉴바 (TOP,BOT)만들기__
          --------
     >    > ＊ __지도 수정사항
          └> - [ ] 지도에서 마커를 클릭하면 세부정보를 띄우기 
          └> - [ ] 카테고리별 선택지를 둬서 필터맞추기 or 검색기능으로 맞추기 
          └> - [ ] 지도에 오버레이 추가__
          ---------
     >    >  ＊__로그인/회원가입 수정사항
          └> - [ ] 간편로그인(카카오이용) 및 아이디/비번찾기 
          └> - [ ] 로그인 페이지 뒷배경추가 및 푸터를 맨밑에 붙이기 
          └> - [ ] 회원가입에 테두리 색 추가__ 
          └> - [ ] 아이디/비번에 제약조건 추가 
          └> - [ ] 마이페이지 - 디자인 추가 
